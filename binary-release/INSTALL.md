...
| `google-research-mcp-tray.exe` | Background HTTP service helper (only needed for HTTP mode) |
| `INSTALL.md` | This file |
| `HTTP_SETUP.md` | Optional: run as a background HTTP service (for LM Studio, etc.) |
...
A native installer dialog opens. Paste your SerpAPI key, optionally tick "enable HTTP service", and click Install. It will:
1. Install 6 skills into `~/.claude/skills/`
2. Install 1 deep-research agent into `~/.claude/agents/`
3. Save your key to `~/.google-research-mcp/config.toml`
4. (If HTTP is enabled) register the background HTTP service to start on login
5. Show the config snippets you need for Step 4

If you'd rather use the terminal, run `google-research-mcp.exe init` from PowerShell to install just the skills + agent.
...
| **Tool calls time out (`MCP error -32001`)** | Your MCP client's per-tool-call timeout is too short. SerpAPI normally responds in 1-2s but occasionally spikes to 5-7s. Set the per-tool-call timeout to **at least 30 seconds**. See "Performance & timeouts" below. |
| Search works in Claude Desktop but not LM Studio | LM Studio's per-tool timeout default is shorter — bump it in MCP settings |

---

## Performance & timeouts

The binary is fast (1-3 ms overhead), but SerpAPI itself is the bottleneck:

| Operation | Typical | Slow path |
|-----------|---------|-----------|
| `google_search` | 1-2 s | up to 7 s on cold queries |
| `extract_webpage_content` | 1-3 s | up to 10 s for slow sites |
| `extract_multiple_webpages` (5 URLs) | 3-5 s | up to 15 s |
| `research_topic` (intermediate) | 8-15 s | up to 30 s |

**Configure your MCP client's per-tool-call timeout to at least 30 seconds.**

The binary already retries SerpAPI once on transient network errors, so most spikes are handled internally. But the retry only helps if the client gives the binary enough time to do it.

### Recommended client timeouts

**Claude Desktop:** uses a generous default — usually no config needed.

**LM Studio:**
```json
"google-research": {
  "command": "C:\\MCP\\google-research-mcp\\binary-release\\google-research-mcp.exe",
  "env": { "SERPAPI_KEY": "..." },
  "timeout": 120
}
```
The `"timeout": 120` controls server startup. Look in LM Studio's Integrations panel for a separate **per-tool-call** timeout slider — this is the one that matters for searches.

### Diagnosing timeouts

Run the binary's self-test from a terminal:

```powershell
$env:SERPAPI_KEY = "your_key"
C:\MCP\google-research-mcp\binary-release\google-research-mcp.exe diagnose
```

Output includes timing for the live SerpAPI call. If it consistently shows >5 s, your network or SerpAPI's region is slow — bump client timeouts higher.

When tools fail in your MCP client, also check the client's **MCP server log panel** for stderr output from the binary. Each tool call logs:
- `google_search received` — proves the binary got the request
- `google_search completed elapsed_ms=...` — proves the binary returned successfully

If the client times out but you see both lines, the problem is purely the client's timeout setting.
